...
export default function MantenimientoPage() {
    ...
    const filtered = useMemo(() => {
        ...
        return (tickets || []).filter((t: Ticket) => {
            ...
            if (misTareas) return t.responsable_id === user?.usuario_id
    ...
    }, [tickets, q, filterStatus, filterPriority, misTareas])
    ...
    return (
        <div className="w-full flex flex-col gap-6">
            ...
            <div className="w-full">
                {/* Pestañas */}
                <div className="flex space-x-1 relative z-10 -mb-[1px]">
                    <button
                        onClick={() => setMisTareas(false)}
                        className={`px-4 py-2 text-sm font-medium cursor-pointer rounded-t-xl transition-colors ${misTareas === false
                            ? 'bg-white text-slate-900 border-t border-x border-slate-300 border-b-white'
                    ...
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium cursor-pointer rounded-t-xl transition-colors ${misTareas === true
                            ? 'bg-white text-slate-900 border-t border-x border-slate-300 border-b-white'
                            : 'bg-transparent text-slate-500 hover:text-slate-700'
                            }`}
                    >
                ...
                {/* Tarjeta de la Tabla */}
                <div className="bg-white rounded-b-xl rounded-tr-xl rounded-tl-none border border-slate-300 px-5 pt-5 pb-5 shadow-sm relative z-0">
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                        <div className="flex items-center gap-3 w-full md:w-1/2">
                            <input
                                value={q}
                                onChange={(e) => setQ(e.target.value)}
                                placeholder="Buscar por ticket, título, descripción, habitación o responsable"
                                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:ring-2 focus:ring-slate-900"
                            />
                        </div>
